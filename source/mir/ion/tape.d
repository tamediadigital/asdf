...
@safe pure nothrow @nogc:
    ...
    ///
    SmallFloat opBinary(string op : "*")(SmallFloat rhs) const
    {
        auto lhs = this;
        ...
        {
        P:
            auto c = cast(int)ctlz(mulResult.high);
            mulResult.high <<= c;
            auto d = T.sizeof * 8 - c;
            resultExp += d;
        ...
        if (overflow)
            mulResult.high = half;
        return SmallFloat(lhs.sign ^ rhs.sign, resultExp, mulResult.high);
    }
